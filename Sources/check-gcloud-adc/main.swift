...
if args.contains("--help") || args.contains("-h") {
    ...
    print("Clicking the notification runs gcloud auth application-default login with")
    print("the required scopes to re-authenticate.")
    print()
    ...
    print()
    print("The gcloud landing page is rewritten before each run so that it stops on")
    print("localhost instead of redirecting to cloud.google.com, where a shared exit")
    print("IP can land on a CAPTCHA that the flow never gets past. Set")
    print("CHECK_GCLOUD_ADC_LANDING_PAGE to an empty string to leave the page alone,")
    print("or CHECK_GCLOUD_ADC_SDK_ROOT to point at the SDK explicitly.")
    print()
