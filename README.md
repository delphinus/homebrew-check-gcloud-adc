...
- gcloud が終わったら、起動した Chrome だけを終了させる。gcloud が途中で落ちた場合もウィンドウは残らない。

gcloud へは `$BROWSER` と、`PATH` に差し込んだ `open` の 2 経路で渡している。前者は gcloud が使う Python の `webbrowser` モジュールが、後者は `open <url>` を直に叩く実装が拾う。

#### 毎回まっさらな状態から始める

`open check-gcloud-adc://reauth` は、何度叩いても常に新しいプロセスから始まる。前回の再認証が残っていれば、始める前に落とす。

これが要るのは、**専用ウィンドウを閉じただけでは再認証が終わらない**ため。`gcloud` はローカルのコールバックを待ち続け、それを待つラッパーの `zsh` も残る。放っておくと叩くたびに溜まっていく。

- ラッパーの `zsh` はプロセスグループのリーダーで、`gcloud` は同じグループにいる。`zsh` だけを落とすと `gcloud` が孤児になって待ち続けるので、**グループごと**落とす。
- 併せて、ウィンドウを閉じたら再認証自体を終わらせるようにした。shim が起動した Chrome の PID を記録しておき、それが消えたらプロセスグループごと終了する。

前回の残骸を探す処理をシェル側ではなく Swift 側に置いているのは、`pgrep`/`pkill` に渡すパターンが自分自身の argv に含まれないようにするため。同じことをラッパーの中でやると `pkill -f` が自分を撃つ。同じ理由で、ラッパーの中では Chrome をコマンドラインのパターンではなく PID で扱っている。
...
# 前回の再認証が残っていれば落とすので、何度叩いても毎回まっさらから始まります
